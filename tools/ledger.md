...
The Ledger is private — it runs on a server only I touch. The same method is captured for everyone else in [Splinched]({{ "/splinched" | relative_url }}), a book + spreadsheet bundle that does what this app does without any of the code. The book carries the story and the system; the spreadsheet handles Accounts, Snapshots, Cash Runway, Recurring expenses, and Milestones; the avalanche-snowball projection + sandbox what-if runs in a separate browser tool at [aaronaiken.me/splinched/projection/]({{ "/splinched/projection/" | relative_url }}), and the on-demand Leak Hunt — bank-CSV in, recurring-charge audit out — runs in its own browser tool at [aaronaiken.me/splinched/leak-hunt/]({{ "/splinched/leak-hunt/" | relative_url }}). Both pages read your file locally — nothing uploads, nothing leaves your tab, the math runs in JavaScript ported one-to-one from the same `helpers/ledger.py` / `helpers/leak_hunt.py` the Flask app uses. Disconnect your wifi and they still work.
